...
export const NawawiFortyView: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">{hadiths.map((h,i) => { const n=Number(h.hadithNumber)||i+1; return <button key={h.id} type="button" onClick={()=>setSelected(n)} className={`ikp-focus-ring min-w-[48px] py-2 px-3 rounded-2xl text-xs font-bold ${Number(current.hadithNumber)===n?'bg-[#176B4D] text-white':'bg-[#E8EFEA] dark:bg-[#252F28]'}`}>{toBengaliNumerals(n)}</button>; })}</div>
    <div className="rounded-3xl p-6 bg-white dark:bg-[#1A221C] border border-[#E8EFEA] shadow-md space-y-5">
      <div className="flex items-start justify-between border-b border-[#E8EFEA] pb-3"><div><span className="text-xs font-bold text-[#176B4D]">হাদিস নং: {toBengaliNumerals(Number(current.hadithNumber))}</span><h3 className="font-extrabold text-base mt-1">ইমাম নববীর ৪০ হাদিস</h3></div><button type="button" onClick={copy} className="ikp-focus-ring p-2.5 rounded-xl">{copied?<Check className="w-4 h-4 text-emerald-600"/>:<Copy className="w-4 h-4"/>}</button></div>
      ...
      <div className="flex justify-end"><button type="button" onClick={()=>{const i=hadiths.indexOf(current);setSelected(Number(hadiths[(i+1)%hadiths.length].hadithNumber));}} className="ikp-focus-ring flex items-center gap-1 px-4 py-2 rounded-xl bg-[#176B4D] text-white text-xs font-bold">পরবর্তী হাদিস <ChevronRight className="w-4 h-4"/></button></div>
    </div>
  </div>;
};
