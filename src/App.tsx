...
const SEARCH_MODULES: SearchModule[] = [
  { path: '/prayer', titleBn: 'নামাজের সময় ও কিবলা', titleEn: 'Prayer', keywords: 'নামাজ সালাত ওয়াক্ত ফজর যোহর আসর মাগরিব এশা সেহরি ইফতার prayer salah', category: 'ibadah', icon: Clock },
  { path: '/qibla', titleBn: 'কিবলা কম্পাস', titleEn: 'Qibla', keywords: 'কিবলা কাবা দিকনির্ণয় compass qibla', category: 'ibadah', icon: Compass },
  { path: '/dua', titleBn: 'দোয়া ও যিকির', titleEn: 'Dua', keywords: 'দোয়া যিকির সকাল সন্ধ্যা তাসবীহ হিসনুল dua dhikr zikr', category: 'ibadah', icon: MoonIcon },
  { path: '/hadith', titleBn: 'হাদিস ও সুন্নাহ', titleEn: 'Hadith', keywords: 'হাদিস বুখারী মুসলিম নববী সুন্নাহ hadith bukhari muslim', category: 'ilm', icon: BookMarked },
  { path: '/quran', titleBn: 'কুরআন', titleEn: 'Quran', keywords: 'কুরআন কোরআন তিলাওয়াত সূরা আয়াত quran surah', category: 'ilm', icon: BookOpen },
  { path: '/learn/salah', titleBn: 'সালাত ও অজু শিক্ষা', titleEn: 'Salah guide', keywords: 'অজু ওজু নামাজ শিক্ষা রাকাত wudu salah learn', category: 'ilm', icon: Sparkles },
  { path: '/zakat', titleBn: 'যাকাত ক্যালকুলেটর', titleEn: 'Zakat', keywords: 'যাকাত নিসাব হিসাব zakat', category: 'tools', icon: Calculator },
  { path: '/calendar', titleBn: 'হিজরি ক্যালেন্ডার', titleEn: 'Calendar', keywords: 'হিজরি চাঁদ ক্যালেন্ডার তারিখ hijri calendar', category: 'tools', icon: Calendar },
  { path: '/ramadan', titleBn: 'রমজান', titleEn: 'Ramadan', keywords: 'রমজান রোজা সেহরি ইফতার ramadan', category: 'ibadah', icon: MoonIcon },
  { path: '/hajj', titleBn: 'হজ ও উমরাহ', titleEn: 'Hajj', keywords: 'হজ উমরাহ hajj umrah', category: 'ilm', icon: Compass },
  { path: '/seerah', titleBn: 'সীরাহ', titleEn: 'Seerah', keywords: 'সীরাহ রাসূল জীবনী seerah', category: 'ilm', icon: BookOpen },
  { path: '/quiz', titleBn: 'ইসলামিক কুইজ', titleEn: 'Quiz', keywords: 'কুইজ প্রশ্ন quiz', category: 'ilm', icon: Sparkles },
];
...
const findQuranSurahs = (term: string) => {
  ...
  return Array.from({ length: 114 }, (_, i) => findQuranSurah(i + 1)).filter((s): s is Surah => {
    if (!s) return false;
    return `${s.number} ${s.nameBengali} ${s.nameEnglish} ${s.nameArabic}`.toLowerCase().includes(q);
  });
};
...
export const App: React.FC = () => {
  ...
  const applyRoute = () => {
    ...
    if (path.startsWith('/quran/')) {
      ...
      if (Number.isInteger(n) && findQuranSurah(n)) { setSelectedTab(1); setSelectedSurahNumber(n); return; }
    }
    ...
    if (path === '/ramadan') { setActiveSpecialModule('RAMADAN'); return; }
  ...
  return (
    <div className="min-h-screen bg-[var(--ikp-bg)] text-[var(--ikp-text)] transition-colors flex flex-col">
          ...
          <div className="max-w-2xl mx-auto mt-4 rounded-3xl ikp-surface shadow-2xl overflow-hidden">
            ...
            <div className="max-h-[70vh] overflow-y-auto p-3 space-y-4">
              ...
              {searchModules.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-xs font-bold text-[#717A74] dark:text-[#8B958E] px-1">{searchTerm ? 'ফিচার ও টুলস' : 'সব ফিচার'}</p>
                  {searchModules.map((m) => {
                    const Icon = m.icon;
                    return (
      ...
      {!showWebModules && !isReaderOpen && (
        <Navbar selectedTab={activeSpecialModule ? -1 : selectedTab} onSelectTab={(idx) => navigate(['/', '/quran', '/prayer', '/dua', '/hadith'][idx] || '/')} />
      )}
      <PwaInstallPrompt />
    </div>
  );
