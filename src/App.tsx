...
export const App:React.FC=()=>{
 ...
 const [showWebModules,setShowWebModules]=useState(false),[activeWebModule,setActiveWebModule]=useState<WebModule|null>(null),[searchOpen,setSearchOpen]=useState(false),[search,setSearch]=useState('');
 ...
 return <div className="min-h-screen bg-[#F7FAF7] dark:bg-[#101511] text-[#181D19] dark:text-[#E1E5E1] transition-colors flex flex-col">
  {!isReaderOpen&&!activeSpecialModule&&<header className="sticky top-0 z-30 bg-[#F7FAF7]/95 dark:bg-[#101511]/95 backdrop-blur border-b border-[#E8EFEA] dark:border-[#3A4D43]/60 px-4 py-2.5"><div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="font-bold text-base sm:text-lg tracking-tight text-[#176B4D] dark:text-[#9DD6B9]">ইসলামিক জ্ঞান</span><span className="hidden sm:inline text-xs px-2 py-0.5 rounded-full bg-[#D4F2E2] text-[#002114] dark:bg-[#005236] dark:text-[#D4F2E2] font-semibold">Web</span></div><div className="flex items-center gap-2"><button type="button" onClick={()=>setSearchOpen(true)} aria-label="ইসলামিক অনুসন্ধান খুলুন" className="p-2 rounded-xl text-[#414A45] dark:text-[#C1CAC4] hover:bg-[#E8EFEA] dark:hover:bg-[#3F4943] transition-colors"><Search className="w-5 h-5"/></button><a {...navLink('/qibla')} aria-label="কিবলা খুলুন" className="px-3 py-2 rounded-xl bg-[#D4F2E2] dark:bg-[#005236] text-[#176B4D] dark:text-[#D4F2E2] text-xs font-bold">কিবলা</a><button type="button" onClick={()=>setIsDarkMode(v=>!v)} aria-label={isDarkMode?'লাইট মোড':'ডার্ক মোড'} className="p-2 rounded-xl text-[#414A45] dark:text-[#C1CAC4] hover:bg-[#E8EFEA] dark:hover:bg-[#3F4943] transition-colors">{isDarkMode?<Sun className="w-5 h-5"/>:<Moon className="w-5 h-5"/>}</button></div></div></header>}
  {searchOpen&&<div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="ইসলামিক অনুসন্ধান"><div className="max-w-2xl mx-auto mt-4 rounded-3xl bg-white dark:bg-[#1A221C] shadow-2xl border border-[#E8EFEA] dark:border-[#3A4D43] overflow-hidden"><div className="flex items-center gap-2 p-3 border-b border-[#E8EFEA] dark:border-[#3A4D43]"><Search className="w-5 h-5 text-[#176B4D]"/><input autoFocus value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')setSearchOpen(false)}} placeholder="কুরআন, দোয়া, হাদিস, কিবলা... খুঁজুন" className="flex-1 bg-transparent outline-none text-sm"/><button type="button" onClick={()=>setSearchOpen(false)} aria-label="অনুসন্ধান বন্ধ করুন"><X className="w-5 h-5"/></button></div><div className="max-h-[70vh] overflow-y-auto p-3 space-y-2">{!searchTerm&&<p className="text-xs text-[#717A74] px-2 py-3">সূরা, ইসলামিক ফিচার বা মডিউলের নাম দিয়ে খুঁজুন।</p>}{searchSurahs.map(s=><button key={s.number} type="button" onClick={()=>{setSearchOpen(false);navigate('/quran/'+s.number)}} className="w-full text-left p-3 rounded-2xl hover:bg-[#F0F5F1] dark:hover:bg-[#222C25]"><div className="font-bold text-sm">{s.nameBengali}</div><div className="text-[11px] text-[#717A74]">সূরা {s.number} • {s.nameEnglish}</div></button>)}{searchModules.map(([path,bn,en])=><button key={path} type="button" onClick={()=>{setSearchOpen(false);navigate(path)}} className="w-full text-left p-3 rounded-2xl hover:bg-[#F0F5F1] dark:hover:bg-[#222C25]"><div className="font-bold text-sm">{bn}</div><div className="text-[11px] text-[#717A74]">{en}</div></button>)}{searchTerm&&!searchSurahs.length&&!searchModules.length&&<p className="text-sm text-center text-[#717A74] py-8">কোনো ফল পাওয়া যায়নি।</p>}</div></div></div>}
