...
export async function fetchHijriMonth(year:number,month:number,latitude=22.3569,longitude=91.7832,method=1){const r=await fetch(`${ROOT}/hijriCalendar/${year}/${month}?latitude=${latitude}&longitude=${longitude}&method=${method}`,{cache:'no-store'});if(!r.ok)throw Error('Hijri calendar unavailable');return (await r.json()).data;}
export {MONTHS};