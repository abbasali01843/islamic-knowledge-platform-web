...
function emptyTracker(dateKey: string): DailySalahTracker {
  return {
    date: dateKey,
    fajr: false,
    dhuhr: false,
    ...
    return {
      location:
        ...
        typeof data.location.latitude === 'number'
          ? (data.location as LocationConfig)
          : null,
      madhab: data.madhab === 'STANDARD' ? 'STANDARD' : 'HANAFI',
      calcMethod: data.calcMethod === 'MWL' ? 'MWL' : 'IFB',
    };
  } catch {
...
export function loadSalahTracker(dateKey: string): DailySalahTracker {
  if (typeof window === 'undefined') return emptyTracker(dateKey);
  try {
    const raw = localStorage.getItem(TRACKER_KEY);
    if (!raw) return emptyTracker(dateKey);
    const data = JSON.parse(raw) as DailySalahTracker;
    ...
    return emptyTracker(dateKey);
  } catch {
    return emptyTracker(dateKey);
  }
}

export function saveSalahTracker(tracker: DailySalahTracker): void {
  if (typeof window === 'undefined') return;
  try {
